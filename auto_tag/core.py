...
from auto_tag import constants, git_custom_env, tag_search_strategy
...
class AutoTag:
    ...
    def get_latest_tag(
        self, repo: git.Repo
    ) -> tuple[Optional[git.refs.tag.TagReference], Optional[semantic_version.Version]]:
        ...
        """
        raw_tag = self._search_strategy(repo=repo, branch=self._branch)
        if raw_tag is None:
            return None, None
        sem_tag = semantic_version.Version(tag_search_strategy.clean_tag_name(str(raw_tag)))
        return raw_tag, sem_tag

    @staticmethod
    def bump_tag(
        tag: Optional[semantic_version.Version], change_type: int
    ...
    def get_all_commits_from_a_tag(
        self, repo: git.Repo, branch: str, tag: Optional[git.refs.tag.TagReference]
        ...
        self._logger.debug("Commits found from after tag %s: %s", tag, commits)
        return commits
    ...
    @staticmethod
    def get_remote(repo: git.Repo, name: str) -> Optional[git.remote.Remote]:
    ...
    def push_to_remotes(self, repo: git.Repo, tag: str) -> None:
        ...
        if self._upstream_remotes:
            self._logger.info("Start pushing to remotes: %s.", self._upstream_remotes)
        else:
        ...
        for remote_name in self._upstream_remotes:
            ...
            if remote:
                ...
                self._logger.error("Can't find remote with name `%s`", remote_name)

    @staticmethod
    def _create_tag_message(
        commits: list[git.objects.commit.Commit], tag: semantic_version.Version
    ) -> str:
        """Create a tag message that contains informations
        from the commits."""

        tag_message = f"Release {str(tag)} \n\n"

        for message in [c.message for c in commits]:
            msg_str = message if isinstance(message, str) else message.decode("utf-8")
            tag_message += f"    * {msg_str.split(chr(10))[0].strip()}\n"
        return tag_message

    @staticmethod
    def _is_last_commit_already_tagged(
        repo: git.Repo, last_tag: Optional[git.refs.tag.TagReference], branch_name: str
    ) -> bool:
        """Check if the last_tag is also applied on the latest commit."""
        if last_tag is None:
            return False
        commit = list(repo.iter_commits(rev=branch_name))[0]
        return bool(last_tag.commit == commit)
    ...
    def work(self) -> None:
        ...
        commits = self.get_all_commits_from_a_tag(repo, self._branch, last_tag)
        ...
        tag = f"v{next_tag}" if self._append_v else str(next_tag)

        self._logger.info("Bumping tag %s -> %s", last_tag, next_tag)

        with git_custom_env.GitCustomeEnvironment(
            str(repo.working_dir), self._git_name, self._git_email
        ):
            tag_on_last_commit = self._is_last_commit_already_tagged(repo, last_tag, self._branch)

            if self._skip_if_exists and tag_on_last_commit:
                self._logger.info(
                    "The tag is already tagged, following your CLI option we will skip tagging."
                )
            else:
                repo.create_tag(str(tag), message=self._create_tag_message(commits, next_tag))

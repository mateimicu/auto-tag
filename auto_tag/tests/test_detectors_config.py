...
from auto_tag import detectors, detectors_config
...
def test_detector_config() -> None:
    ...
    path_to_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "data", "detectors_example.yaml"
    )
    ...
    for detector in config.detectors:
        if detector.name == "detector_1":
            assert isinstance(detector, detectors.CommitMessageHeadStartsWithDetector)
        ...
        if detector.name == "detector_4":
            assert isinstance(detector, detectors.CommitMessageContainsDetector)

...
from auto_tag.detectors import CommitMessageContainsDetector, CommitMessageHeadStartsWithDetector
...
def test_simple_flow_no_existing_tag(simple_repo: str) -> None:
    ...
    entrypoint.main(["-r", simple_repo, "-b", "master", "--name", TEST_NAME, "--email", TEST_EMAIL])
...
def test_simple_flow_existing_tag(
    ...
    default_detectors: list[
        Union[CommitMessageContainsDetector, CommitMessageHeadStartsWithDetector]
    ],
) -> None:
    """Test a simple flow locally."""
    repo = git.Repo(simple_repo, odbt=git.GitDB)
...
def test_simple_flow_existing_tag_minor_bump(
    existing_tag: str, next_tag: str, simple_repo_minor_commit: str
) -> None:
    """Test a simple flow locally."""
    ...
    entrypoint.main(
        ["-r", simple_repo_minor_commit, "-b", "master", "--name", TEST_NAME, "--email", TEST_EMAIL]
    )
...
def test_simple_flow_existing_tag_major_bump(
    ...
    entrypoint.main(
        ["-r", simple_repo_major_commit, "-b", "master", "--name", TEST_NAME, "--email", TEST_EMAIL]
    )
...
def test_tag_message_user_exists_and_specified(simple_repo: str) -> None:
    ...
    entrypoint.main(
        ["-r", simple_repo, "-b", "master", "--name", TEST_NAME_2, "--email", TEST_EMAIL_2]
    )
...
def test_tag_message_user_exists_and_only_email_specified(
    ...
    entrypoint.main(["-r", simple_repo, "-b", "master", "--email", TEST_EMAIL_2])
